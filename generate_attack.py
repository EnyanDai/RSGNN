...
surrogate = GCN(nfeat=features.shape[1], nclass=labels.max().item()+1, nhid=args.hidden,
...
def main():
    ...
    import os
    root = './data/{}'.format(args.label_rate)
    if not os.path.exists(root):
        os.makedirs(root)
    model.save_adj(root=root, name='{}_meta_adj_{}'.format(args.dataset,args.ptb_rate))
